...
dat_seasonal[startsWith(variable, "SCD")] %>% 
  dcast(Name + year ~ variable, value_var = "value") %>% 
  .[, .(SCD_MAM = sum(SCD_MAM >= 92) / 49, 
        SCD_NDJF = sum(SCD_NDJF >= 120) / 49,
        SCD_NDJFMAM = sum(SCD_NDJFMAM >= 212) / 49),
    .(Name)] %>% 
  melt(id.vars = "Name") %>% 
...
ggsave(gg,
       filename = "/mnt/CEPH_PROJECTS/ALPINE_WIDE_SNOW/PAPER/fig-pdf/Figure C2.png",
       width = 10,
       height = 11)
...
read_docx() %>% 
  body_add_flextable(ft) %>% 
  print(target = "/mnt/CEPH_PROJECTS/ALPINE_WIDE_SNOW/PAPER/03_review2/table/trends-gls-seasonalSCD-1000m.docx")
