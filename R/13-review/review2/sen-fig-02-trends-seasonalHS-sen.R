...
ggsave(gg,
       filename = "/mnt/CEPH_PROJECTS/ALPINE_WIDE_SNOW/PAPER/03_review2/fig/trends-seasonalHS-sen.png",
       width = 10,
       height = 11)
...
ft <- dat_table_out3 %>% 
  ...
  theme_booktabs() %>%
  hline(i = 1:4*5, border = fp_border()) %>% 
  vline(j = 2 + 0:5*3, border = fp_border()) %>% 
  merge_h(part = "header") %>%
...
read_docx() %>% 
  ...
  print(target = "/mnt/CEPH_PROJECTS/ALPINE_WIDE_SNOW/PAPER/03_review2/table/trends-sen-seasonalHS-500m.docx")
...
ft <- dat_table_out3 %>% 
  ...
  theme_booktabs() %>%
  hline(i = 1:4*5, border = fp_border()) %>% 
  vline(j = 2 + 0:2*3, border = fp_border()) %>% 
  merge_h(part = "header") %>%
  # align(align = "left", part = "all") %>%
  align(j = dat_header[1:2, col_keys], align = "left") %>% 
  align(j = dat_header[-c(1:2), col_keys], align = "center", part = "all") %>% 
...
read_docx() %>% 
  body_add_flextable(ft) %>% 
  print(target = "/mnt/CEPH_PROJECTS/ALPINE_WIDE_SNOW/PAPER/03_review2/table/trends-sen-seasonalHS-1000m.docx")
