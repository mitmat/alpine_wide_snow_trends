...
dat_seasonal <- readRDS("/mnt/CEPH_PROJECTS/ALPINE_WIDE_SNOW/NIMBUS/SCD_ge_1cm/data-02-seasonalSCD.rds")
load("/mnt/CEPH_PROJECTS/ALPINE_WIDE_SNOW/NIMBUS/SCD_ge_1cm/trends-01-1971-2019-ols-gls.rda")
...
dat_plot_full[, variable_fct := fct_relevel(factor(variable), "SCD_NDJF")]
dat_plot_full
...
gg <-
  ...
  scale_x_continuous(breaks = seq(-30, 20, by = 5))+
  scale_y_continuous(breaks = seq(0, 3000, by = 500), limits = c(0, NA))+
  geom_blank(inherit.aes = F, data = dat_ylim, aes(x = 0, y = max_elev))+
  xlab("Linear trend in seasonal SCD indices [days per decade]")+
  ylab("Elevation [m]")


# ggsave(gg,
#        filename = "/mnt/CEPH_PROJECTS/ALPINE_WIDE_SNOW/PAPER/03_review2/fig/trends-seasonalSCD-GLS.png",
#        width = 10,
#        height = 11)


ggsave(gg,
       filename = "/mnt/CEPH_PROJECTS/ALPINE_WIDE_SNOW/NIMBUS/SCD_ge_1cm/Figure C2.png",
       width = 10,
       height = 11)
...
#                            .(nn = as.double(.N),
#                              mu = 10*mean(estimate)),
#                            .(variable_fct, elev_fct, cluster_fct)]
# 
# 
# dat_table_sig <- dat_table[, 
#                            .(nn = .N), 
#                            .(variable_fct, elev_fct, cluster_fct, p_sig = p.value < 0.05, trend_sign)]
# dat_table_sig[, perc := nn / sum(nn), .(variable_fct, elev_fct, cluster_fct)]
# 
# dat_table_sig[p_sig == T] %>% 
#   dcast(variable_fct + elev_fct + cluster_fct ~ trend_sign, value.var = "perc") %>% 
#   merge(dat_table_avg, all = T) -> dat_table_out
# 
# 
# dat_table_out2 <- dat_table_out %>% 
#   melt(id.vars = c("variable_fct", "elev_fct", "cluster_fct"))
# dat_table_out2[, variable := factor(variable, levels = c("nn", "mu", "neg", "pos"))]
# setorder(dat_table_out2, variable_fct, elev_fct, cluster_fct, variable)
# 
# dat_table_out3 <- dat_table_out2 %>% 
#   dcast(variable_fct + cluster_fct ~ elev_fct + variable, value.var = c("value"))
# 
...
# dat_header[1:2, ":="(row1 = c("Index", "Region"), row2 = c("", ""))]
...
#   theme_booktabs() %>%
#   hline(i = 1:3*5, border = fp_border()) %>% 
...
#   merge_v(j = "variable_fct") %>%
#   valign(j = "variable_fct", valign = "top") %>%
#   fix_border_issues() %>% 
...
#   print(target = "/mnt/CEPH_PROJECTS/ALPINE_WIDE_SNOW/PAPER/03_review2/table/trends-gls-seasonalSCD-500m.docx")
# 
# 
# 
# 
...
ft <- dat_table_out3 %>% 
  ...
  set_header_df(dat_header) %>% 
  colformat_int(j = dat_header[row2 == "#", col_keys]) %>% 
  colformat_double(j = dat_header[row2 == "mean", col_keys], digits = 2) %>% 
  # colformat_num(j = dat_header[row2 == "mu", col_keys]) %>% 
  theme_booktabs() %>%
  hline(i = 1:3*5, border = fp_border()) %>% 
...
read_docx() %>% 
  body_add_flextable(ft) %>% 
  print(target = "/mnt/CEPH_PROJECTS/ALPINE_WIDE_SNOW/NIMBUS/SCD_ge_1cm/appendix-table-C2.docx")
