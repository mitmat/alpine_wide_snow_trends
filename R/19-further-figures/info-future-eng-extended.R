...
gg <- dat_dp[elev %in% elev_plot] %>% 
  ggplot(aes(xx_plot, yy+0.5))+
  geom_hline(yintercept = 0:12*3, colour = grey(0.8), linewidth = 0.2)+ # 1 month
  
  geom_point(shape = "\u2744", size = 2.5, colour = "#2171b5")+
  ...
  theme(plot.background = element_rect(colour = "white", fill = "white"),
        plot.subtitle = element_text(colour = "grey40", size = rel(11/14)))+
  scale_x_continuous(NULL, limits = c(0, 3000), expand = c(0,0),
  ...
  ggtitle("Days with snow on ground in the Alps",
          "Impact of global warming and climate action on \n end of century (2071-2100) snow cover")+
  
  
  #legend
  annotate("rect", xmin = 50, xmax = 2500, ymin = 28.5, ymax = 37.5, 
           colour = "white", fill = "white")+
  annotate("point", 100, 36+1, shape = "\u2744", size = 5, colour = "#2171b5")+
  annotate("text", 150, 36+1, hjust = 0, vjust = 0.5, size = 3, 
           label = "Day with snow on ground, recent (2001-2020)", colour = "#2171b5")+
...
ggsave(gg, filename = "fig/future-eng-03-full.png",
       width = 6, height = 4, units = "in")

# cairo_pdf("fig/info-future_DE.pdf", width = 7, height = 4)
# gg
# dev.off()
...
ggsave(gg, filename = "fig/future-eng-01-past.png",
       width = 6, height = 4, units = "in")
...
ggsave(gg, filename = "fig/future-eng-02-paris.png",
       width = 6, height = 4, units = "in")
