...
load("~/projects/ALPINE_WIDE_SNOW/PAPER/02_review/rds/data4corr-01-merged.rda")
...
gg1 <- dat_plot[season == "DJF"] %>% 
  ...
  # facet_grid(. ~ season, scales = "free")+
  ylab("Elevation [m]")+
  xlab("Average winter snow depth (Dec-Feb) [cm]")

gg_out <- gg1+inset_element(gg_icon_snow, 0.7, 0.0, 1, 0.4)

ggsave(filename = "fig/clim-hs-DJF_EN.png",
       gg_out,
       width = 6, height = 4)
...
gg1 <- dat_plot[season == "DJF"] %>% 
  ...
  # facet_grid(. ~ season, scales = "free")+
  ylab("Elevation [m]")+
  xlab("Average winter temperature (Dec-Feb) [°C]")

gg_out <- gg1+inset_element(gg_icon_tmean, 0.0, 0.0, 0.3, 0.4)


ggsave(filename = "fig/clim-tmean-DJF_EN.png",
       gg_out,
       width = 6, height = 4)
...
gg1 <- dat_plot[season == "DJF"] %>% 
  ...
  # facet_grid(. ~ season, scales = "free")+
  ylab("Elevation [m]")+
  xlab("Average winter precipitation (Dec-Feb) [mm]")

gg_out <- gg1+inset_element(gg_icon_prec, 0.7, 0.6, 1.0, 1.0)



ggsave(filename = "fig/clim-prec-DJF_EN.png",
       gg_out,
       width = 6, height = 4)
...
ggsave(filename = "fig/clim-prec-DJF-region5_EN.png",
       gg_out,
       width = 8, height = 5)
...
ggsave(filename = "fig/clim-HS-DJF-region5_EN.png",
       gg_out,
       width = 8, height = 5)
...
ggsave(filename = "fig/clim-tmean-DJF-region5_EN.png",
       gg_out,
       width = 8, height = 5)
