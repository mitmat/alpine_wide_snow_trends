...
dat_plot_m[xx <= 400 & yy <= 1000] %>% 
  ggplot(aes(xx+10, yy+25, fill = value_f))+
  geom_tile()+
  theme_bw()+
  ...
  #                   show.limits = T)+
  scale_fill_scico_d("Correlation", 
                     direction = 1, palette = "roma")+
...
dat_plot[xx <= 400 & yy <= 1000] %>% 
  ggplot(aes(xx+10, yy+25, fill = nn))+
  geom_tile()+
  theme_bw()+
  scale_fill_binned("N",
                    type = "viridis",
                    n.breaks = 10,
                    # breaks = c(0, 20, 50, 100, 500, 3000),
                    show.limits = T)+
  # scale_fill_scico("N", 
  #                    direction = 1, palette = "roma")+
  # xlim(0, 400)+ylim(0,1000)+
