...
# v1.3:
# - monthly update with HSmax and SCD**
...
path_out <- "/mnt/CEPH_PROJECTS/ALPINE_WIDE_SNOW/PAPER/zenodo-data-v1.3/"
...
setnames(dat_monthly_01_orig, c("Name", "year", "month", "HNsum", "HSmean"))

dat_monthly_02_orig_more <- rbind(
  readRDS("/mnt/CEPH_PROJECTS/ALPINE_WIDE_SNOW/05_MONTHLY/rds/1787hn_1879hs-1960/data_long_HSmax_SCD.rds"),
  readRDS("/mnt/CEPH_PROJECTS/ALPINE_WIDE_SNOW/05_MONTHLY/rds/1961-2020/data_long_HSmax_SCD.rds")
)


dat_monthly_03_gapfill <- readRDS("/mnt/CEPH_PROJECTS/ALPINE_WIDE_SNOW/05_MONTHLY/rds/1961-2020_gapfilled_HS_sub/data_long_HS.rds")
setnames(dat_monthly_03_gapfill, c("HS"), c("HSmean_gapfill"))



dat_monthly_04_gapfill_more <- readRDS("/mnt/CEPH_PROJECTS/ALPINE_WIDE_SNOW/05_MONTHLY/rds/1961-2020_gapfilled_HS_sub/data_long_HSmax_SCD.rds")
dat_monthly_04_gapfill_more[, frac_gapfilled := NULL]
cols_more <- c("HSmax", "SCD1", "SCD1gt", "SCD10", "SCD20", "SCD30", "SCD50", "SCD100")
setnames(dat_monthly_04_gapfill_more, cols_more, paste0(cols_more, "_gapfill"))
...
for(i_prov in prov_monthly){
  ...
  dat_monthly_01_orig[Name %in% i_stns] %>% 
    merge(dat_monthly_02_orig_more[Name %in% i_stns], by = c("Name", "year", "month"), all = T) %>% 
    merge(dat_monthly_03_gapfill[Name %in% i_stns], by = c("Name", "year", "month"), all = T) %>% 
    merge(dat_monthly_04_gapfill_more[Name %in% i_stns], by = c("Name", "year", "month"), all = T) -> dat_out
