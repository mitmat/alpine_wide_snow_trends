...
dat_ds[experiment == "rcp26", .(institute_rcm, gcm)] %>% unique -> dat_reduced_ensemble
...
dat_dp[elev %in% elev_plot] %>% 
  ...
  geom_label(data = dat_text, 
            ...
            hjust = 0.5, vjust = 0.5, colour = "#9ecae1", size = 3, label.padding = unit(0.15, "lines"))+
  geom_point(data = dat_text, 
             aes(elev - 270 - 60, yy),
             shape = "\u2744", size = 3, colour = "#9ecae1")+
  geom_label(data = dat_text, 
             aes(elev, yy, label = loss_ch),
             hjust = 0.5, vjust = 0.5, colour = "black", size = 3, label.padding = unit(0.15, "lines"))+
  geom_point(data = dat_text, 
             aes(elev - 50, yy),
             shape = 4, size = 2, colour = "black")+
  geom_label(data = dat_text, 
             aes(elev + 250, yy, label = climact_ch),
             hjust = 0.5, vjust = 0.5, colour = "#e6550d", size = 3, label.padding = unit(0.15, "lines"))+
  geom_point(data = dat_text, 
             aes(elev + 250 - 50, yy),
...
ggsave("fig/info-bads-fullred/ds_full.png",
...
dat_ds %>% merge(dat_reduced_ensemble) %>% 
  .[elev > 200 & elev < 3600,
    .(scd = round(mean(scd))),
    .(elev_f, elev, experiment, period, fp)] -> dat_ens_mean
...
ggsave("fig/info-bads-fullred/ds_reduced.png",
       width = 7, height = 4, units = "in")
...
ggsave("fig/info-bads-fullred/ba_full.png",
       width = 7, height = 4, units = "in")
...
dat_bc %>% merge(dat_reduced_ensemble) %>% 
  .[elev > 200 & elev < 3600,
    .(scd = round(mean(snc)*365)),
    .(elev_f, elev, experiment, period, fp)] -> dat_ens_mean
...
ggsave("fig/info-bads-fullred/ba_reduced.png",
       width = 7, height = 4, units = "in")
